...
int valid_input(char* expression){
    ...
    for (int i=1;i<strlen(expression);i++){
        ...
        if (!isdigit(expression[i-1]) && !isalpha(expression[i-1]) && !isoperator(expression[i-1]) && expression[i-1] != '(' && expression[i-1] != ')'){
            printf("%d\n",i);
            printf("%s\n", expression);
            for (int j=0;j<i-1;j++) printf(" "); printf("^\n");
            for (int j=0;j<i-1;j++) printf(" "); printf("Unknown Character!\n");
            return 0;
        }
    ...
    while (1){
        ...
        if (valid_input(expression)){
            ...
            if (digit_input(expression)){
                printf("\n= ");
                // eval using postfix
                char* post = (char*)malloc(sizeof(char) * (strlen(expression))); char** ptrans = &post;
                int c = 0; int* count = &c;
